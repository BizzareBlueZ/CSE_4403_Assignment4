...
struct Vehicle
{
    int wcap, vcap, hcap, start, finish;
};
                     ...
                     int K, ll budget)
{
    int p = hubs.size();
    if (p > 20)
        throw runtime_error("This academic exhaustive hub selector requires P <= 20.");
    ll totalGen = 0, totalPlant = 0;
    for (auto &z : zones)
        totalGen += z.weight;
    for (auto &r : plants)
        totalPlant += r.capacity;
    ...
    for (int mask = 1; mask < (1 << p); ++mask)
    {
        ...
        for (auto &z : zones)
        {
            ...
            for (int hi : selected)
            {
                if (d[z.node][hubs[hi].node] >= INF)
                    continue;
                for (auto &r : plants)
                {
                    if (d[hubs[hi].node][r.node] >= INF)
                        continue;
            ...
            if (z.weight > 0 && bestPath > (INF - fixed - transport) / max(1, z.weight))
            {
                feasible = false;
                break;
            }
                                                      ...
                                                      const vector<Hub> &hubs, int centralNode, const vector<vector<ll>> &d)
{
    vector<int> nodes;
    for (int hi : selected)
        nodes.push_back(hubs[hi].node);
    nodes.push_back(centralNode);
    int n = nodes.size();
    vector<tuple<ll, int, int>> edges;
    for (int i = 0; i < n; i++)
        for (int j = i + 1; j < n; j++)
            if (d[nodes[i]][nodes[j]] < INF)
                edges.emplace_back(d[nodes[i]][nodes[j]], i, j);
    sort(edges.begin(), edges.end());
    DSU dsu(n);
    ll cost = 0;
    vector<tuple<int, int, ll>> used;
    for (auto [w, a, b] : edges)
    {
        if (dsu.unite(a, b))
        {
            cost += w;
            used.emplace_back(nodes[a], nodes[b], w);
                            ...
                            const vector<vector<ll>> &d)
{
    ...
    for (int i = 0; i < Z; i++)
        for (int j = 0; j < H; j++)
        {
            ...
            if (d[zones[i].node][h.node] < INF)
                mf.addEdge(zoneBase + i, hubInBase + j, zones[i].weight);
        }
    for (int j = 0; j < H; j++)
        mf.addEdge(hubInBase + j, hubOutBase + j, hubs[selected[j]].capacity);
    for (int j = 0; j < H; j++)
        for (int r = 0; r < R; r++)
            if (d[hubs[selected[j]].node][plants[r].node] < INF)
                mf.addEdge(hubOutBase + j, plantBase + r, total);
    for (int r = 0; r < R; r++)
        mf.addEdge(plantBase + r, T, plants[r].capacity);
                                       ...
                                       const vector<int> &candidates)
{
    ...
    for (auto &kv : dp)
    {
        if (kv.second.value > best.value ||
            (kv.second.value == best.value && kv.second.picked < best.picked))
            best = kv.second;
    }
                             ...
                             int dayStart, int dayFinish)
{
    vector<int> a;
    for (int i : items)
        if (zones[i].start >= dayStart && zones[i].finish <= dayFinish)
            a.push_back(i);
...
vector<int> stratifiedAuditSample(const vector<Zone> &zones, int quota, unsigned seed)
{
    ...
    while (remaining > 0)
    {
        ...
        for (auto [frac, s] : rem)
        {
            if (q[s] < (int)strata[s].size() && remaining > 0)
            {
                q[s]++;
                remaining--;
                moved = true;
...
int main()
{
    ...
    for (int tc = 1; tc <= T; ++tc)
    {
        ...
        for (auto &z : zones)
        {
            cin >> z.node >> z.weight >> z.volume >> z.hazard >> z.value >> z.risk >> z.start >> z.finish;
            --z.node;
        }
        vector<Hub> hubs(P);
        for (auto &h : hubs)
        {
            cin >> h.node >> h.fixedCost >> h.capacity;
            --h.node;
        }
        vector<Plant> plants(R);
        for (auto &r : plants)
        {
            cin >> r.node >> r.capacity;
            --r.node;
        }
        int K, centralPlantIndex;
        ll budget;
        cin >> K >> budget >> centralPlantIndex;
        --centralPlantIndex;
        int V;
        cin >> V;
        vector<Vehicle> vehicles(V);
        for (auto &v : vehicles)
            cin >> v.wcap >> v.vcap >> v.hcap >> v.start >> v.finish;
        int inspectors, auditQuota;
        unsigned seed;
        cin >> inspectors >> auditQuota >> seed;
        ...
        if (mstCost >= INF)
            cout << "Backbone MST: disconnected\n";
        else
            cout << "Backbone MST cost: " << mstCost << "\n";
